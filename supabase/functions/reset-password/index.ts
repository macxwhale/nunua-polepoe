...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const updatePromises = userIds.map((id) => supabaseAdmin.auth.admin.updateUserById(id, { password: newPin }));
    ...
    try {
      ...
      if (atApiKey && atUsername) {
        ...
        const formattedPhone = phone_number.startsWith("0") ? `+254${phone_number.substring(1)}` : phone_number;
        ...
        console.warn(
          "Africa's Talking credentials not configured (AT_SMS_API_KEY, AT_SMS_USERNAME), skipping SMS notification",
        );
      }
    } catch (smsError) {
